...
(* a function compString : string -> sinstr list that parses a string as 
an expression and compiles it to stack machine code, using the expression 
parser from Parse.fs and the compiler scomp *)
let compString (s: string) : sinstr list = 
      s |> Parse.fromString |> fun e -> scomp e []