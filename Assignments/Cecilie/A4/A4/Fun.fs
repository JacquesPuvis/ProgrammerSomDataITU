...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Call(Var f, eArg) -> 
      ...
      | Closure (f, xs, fBody, fDeclEnv) ->
        ...
        let evalList = List.map (fun e -> Int(eval e env)) eArg // run eval on each argument/eval on each argument
        let paramBindings = List.zip xs evalList //create (param, value) bindings
        let fBodyEnv = (f, fClosure) :: (paramBindings @ fDeclEnv) //concatenates the param bindings with the existing environment
